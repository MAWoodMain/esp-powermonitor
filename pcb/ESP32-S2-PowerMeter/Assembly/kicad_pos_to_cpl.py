...
with open(sys.argv[1], 'r') as in_file, open(sys.argv[2], 'w', newline='') as out_file:
    ...
    for row in reader:
        ...
        try: angle_adjustment = overrides[row['Ref']]['r']
        except KeyError: 
            angle_adjustment = 0.0
        try: x_adjustment = overrides[row['Ref']]['x']
        except KeyError:
            x_adjustment = 0.0
        try: y_adjustment = overrides[row['Ref']]['y']
        except KeyError:
            y_adjustment = 0.0
        writer.writerow({
            'Designator': row['Ref'], 
            'Mid X': str(float(row['PosX'])+x_adjustment) + 'mm', 
            'Mid Y': str(float(row['PosY'])+y_adjustment) + 'mm', 
